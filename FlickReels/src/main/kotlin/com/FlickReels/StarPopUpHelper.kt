...
object StarPopUpHelper {
    ...
    private fun showStyledDialog(activity: Activity) {
        ...
        // ===== Telegram =====
        val telegramView = TextView(activity).apply {
            text = "Join Channel kami di https://t.me/orcaplus"
            ...
            setOnClickListener {
                try {
                    val intent = Intent(Intent.ACTION_VIEW, Uri.parse("https://t.me/orcaplus"))
                    activity.startActivity(intent)
                } catch (e: Exception) {
                    Log.e(TAG, "Error opening telegram link: ${e.message}")
                }
            }
        }
        layout.addView(telegramView)
